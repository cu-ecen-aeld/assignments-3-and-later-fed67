                ...
                loff_t *f_pos)
{
    ...
    size_t offset = *f_pos;
    loff_t size = min(get_total_size(my_ring_buf), count);
    /**
     * TODO: handle read
     */
    ...
    char* buffer_loc = (char*) kmalloc(sizeof(char)*size, GFP_KERNEL);
        if(buffer_loc == NULL) {
        return -ENOMEM;
    }
    ...
    for(size_t i = 0; i < size; ++i ) {
        ...
        struct aesd_buffer_entry* ptr = aesd_circular_buffer_find_entry_offset_for_fpos(my_ring_buf, offset+i, &entry_offset_byte_rtn);
    ...
    if(copy_to_user(buf, buffer_loc, size))
        return -EFAULT; 
